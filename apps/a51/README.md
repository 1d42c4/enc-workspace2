...
build.cmd a51
make-key.cmd a51
dist\a51\a51.exe E input.bin encrypted.bin
dist\a51\a51.exe D encrypted.bin restored.bin
test.cmd a51
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a51` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
