...
build.cmd a95
make-key.cmd a95
dist\a95\a95.exe E input.bin encrypted.bin
dist\a95\a95.exe D encrypted.bin restored.bin
test.cmd a95
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a95` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
