...
build.cmd a56
make-key.cmd a56
dist\a56\a56.exe E input.bin encrypted.bin
dist\a56\a56.exe D encrypted.bin restored.bin
test.cmd a56
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a56` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
