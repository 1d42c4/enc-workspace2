...
**Classification: composed / experimental.** 
...
build.cmd a41
make-key.cmd a41
dist\a41\a41.exe E input.bin encrypted.bin
dist\a41\a41.exe D encrypted.bin restored.bin
test.cmd a41
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a41` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
