...
build.cmd a5
make-key.cmd a5
dist\a5\a5.exe E input.bin encrypted.bin
dist\a5\a5.exe D encrypted.bin restored.bin
test.cmd a5
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a5` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
