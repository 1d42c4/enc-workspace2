...
build.cmd a46
make-key.cmd a46
dist\a46\a46.exe E input.bin encrypted.bin
dist\a46\a46.exe D encrypted.bin restored.bin
test.cmd a46
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a46` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
