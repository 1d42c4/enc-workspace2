...
build.cmd a36
make-key.cmd a36
dist\a36\a36.exe E input.bin encrypted.bin
dist\a36\a36.exe D encrypted.bin restored.bin
test.cmd a36
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a36` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
