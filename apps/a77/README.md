...
build.cmd a77
make-key.cmd a77
dist\a77\a77.exe E input.bin encrypted.bin
dist\a77\a77.exe D encrypted.bin restored.bin
test.cmd a77
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a77` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
