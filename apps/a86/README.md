...
build.cmd a86
make-key.cmd a86
dist\a86\a86.exe E input.bin encrypted.bin
dist\a86\a86.exe D encrypted.bin restored.bin
test.cmd a86
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a86` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
