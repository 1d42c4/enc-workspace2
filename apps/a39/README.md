...
build.cmd a39
make-key.cmd a39
dist\a39\a39.exe E input.bin encrypted.bin
dist\a39\a39.exe D encrypted.bin restored.bin
test.cmd a39
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a39` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
