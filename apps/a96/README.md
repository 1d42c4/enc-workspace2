...
build.cmd a96
make-key.cmd a96
dist\a96\a96.exe E input.bin encrypted.bin
dist\a96\a96.exe D encrypted.bin restored.bin
test.cmd a96
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a96` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
