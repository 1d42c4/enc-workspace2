...
build.cmd a100
make-key.cmd a100
dist\a100\a100.exe E input.bin encrypted.bin
dist\a100\a100.exe D encrypted.bin restored.bin
test.cmd a100
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a100` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
