...
build.cmd a4
make-key.cmd a4
dist\a4\a4.exe E input.bin encrypted.bin
dist\a4\a4.exe D encrypted.bin restored.bin
test.cmd a4
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a4` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
