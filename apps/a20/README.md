...
build.cmd a20
make-key.cmd a20
dist\a20\a20.exe E input.bin encrypted.bin
dist\a20\a20.exe D encrypted.bin restored.bin
test.cmd a20
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a20` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
