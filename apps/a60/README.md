...
**Classification: established primitive.** CBC uses PKCS#7 padding.
...
build.cmd a60
make-key.cmd a60
dist\a60\a60.exe E input.bin encrypted.bin
dist\a60\a60.exe D encrypted.bin restored.bin
test.cmd a60
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a60` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
