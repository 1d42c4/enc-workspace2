...
build.cmd a27
make-key.cmd a27
dist\a27\a27.exe E input.bin encrypted.bin
dist\a27\a27.exe D encrypted.bin restored.bin
test.cmd a27
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a27` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
