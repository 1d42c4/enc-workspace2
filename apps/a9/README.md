...
**Classification: established primitive.** SIV uses two AES keys: the total derived key is twice the AES key size.
...
build.cmd a9
make-key.cmd a9
dist\a9\a9.exe E input.bin encrypted.bin
dist\a9\a9.exe D encrypted.bin restored.bin
test.cmd a9
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a9` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
