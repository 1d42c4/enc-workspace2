...
build.cmd a43
make-key.cmd a43
dist\a43\a43.exe E input.bin encrypted.bin
dist\a43\a43.exe D encrypted.bin restored.bin
test.cmd a43
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a43` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
