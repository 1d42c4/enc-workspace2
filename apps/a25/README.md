...
build.cmd a25
make-key.cmd a25
dist\a25\a25.exe E input.bin encrypted.bin
dist\a25\a25.exe D encrypted.bin restored.bin
test.cmd a25
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a25` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
