...
build.cmd a40
make-key.cmd a40
dist\a40\a40.exe E input.bin encrypted.bin
dist\a40\a40.exe D encrypted.bin restored.bin
test.cmd a40
```

Run the helpers from the workspace root. Copy `input.bin` into `dist\a40` first. Key generation is needed only once; it refuses to replace a key. If you supply your own key file, use 32 or more random bytes. Nonempty raw files up to 1 MiB are accepted, but weak files are still weak. No password stretching is performed.
